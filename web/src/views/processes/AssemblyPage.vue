...
const draftFields = computed<FormField[]>(() => [
  { key: 'casingEquipmentCode', label: '入壳设备编号', type: 'select', options: equipmentOptions.value },
  { key: 'shellModel', label: '壳体型号', type: 'select', options: shellOptions.value },
  { key: 'bottomWeldEquipment', label: '底焊设备编号', type: 'select', options: equipmentOptions.value },
  { key: 'bottomWeldParams', label: '底焊参数' },
  { key: 'capModel', label: '盖板型号', type: 'select', options: capOptions.value },
  { key: 'operatorName', label: '操作员', type: 'select', options: operatorOptions.value },
])
const qualityFields: FormField[] = [
  { key: 'bottomWeldPull', label: '底焊拉力(N)', type: 'number' },
  { key: 'grooveRecord', label: '滚槽记录' },
  { key: 'capWeldingPull', label: '盖板焊接拉力(N)', type: 'number' },
  { key: 'tabWeldingPull', label: '极耳焊接拉力(N)', type: 'number' },
]
