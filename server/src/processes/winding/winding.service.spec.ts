...
describe('WindingService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WindingService,
        { provide: getRepositoryToken(WindingRecord), useValue: repo },
    ...
    service = module.get<WindingService>(WindingService);
  });
  ...
  describe('createDraft', () => {
    ...
    it('should update existing draft instead of creating new', async () => {
      const existingDraft = { ...mockRecord, id: 1, equipmentCode: 'Old-WD' };
      (batchRepo.findOne as jest.Mock).mockResolvedValue(mockBatch);
      (repo.findOne as jest.Mock).mockResolvedValue(existingDraft);
      (repo.save as jest.Mock).mockResolvedValue({ ...existingDraft, equipmentCode: 'WD-001' });

      const result = await service.createDraft(draftDto, 1);
      expect(result.equipmentCode).toBe('WD-001');
    });
  });
  ...
  describe('submitQuality', () => {
    ...
    it('should submit draft with quality fields', async () => {
      ...
      expect(result.isDraft).toBe(false);
      expect(result.coreThickness).toBe(5.5);
    });
