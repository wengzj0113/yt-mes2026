...
describe('InjectionService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InjectionService,
        { provide: getRepositoryToken(InjectionRecord), useValue: repo },
  ...
  describe('createDraft', () => {
    ...
    it('should update existing draft instead of creating new', async () => {
      ...
      (repo.save as jest.Mock).mockResolvedValue({ ...existingDraft, equipmentCode: 'IJ-001' });
      const result = await service.createDraft(draftDto, 1);
      expect(result.equipmentCode).toBe('IJ-001');
    });
  });
  ...
  describe('submitQuality', () => {
    const qualityDto = {
      batchNo: 'WT26A01MA',
      injectionAmount: 5.5,
      injectionHumidity: 1.2,
      injectionTemperature: 25.0,
      sealingDimension: 18.5,
      cleaningRecord: '清洁正常',
    };
    ...
    it('should submit draft with quality fields', async () => {
      ...
      expect(result.isDraft).toBe(false);
      expect(result.injectionAmount).toBe(5.5);
    });
