...
describe('SlittingService', () => {
  let service: SlittingService;
  let repo: jest.Mocked<Partial<Repository<SlittingRecord>>>;
  let batchRepo: jest.Mocked<Partial<Repository<any>>>;
  let qualityCheckRepo: jest.Mocked<Partial<Repository<any>>>;

  const mockRecord: SlittingRecord = {
    id: 1,
    batchNo: 'WT26A01MA',
    recordStatus: 1,
    isDraft: true,
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SlittingService,
        { provide: getRepositoryToken(SlittingRecord), useValue: repo },
    ...
    service = module.get<SlittingService>(SlittingService);
  });
  ...
  describe('createDraft', () => {
    ...
    it('should update existing draft instead of creating new', async () => {
      ...
      (repo.save as jest.Mock).mockResolvedValue({ ...existingDraft, equipmentCode: 'SL-001' });

      const result = await service.createDraft(draftDto, 1);
      expect(result.equipmentCode).toBe('SL-001');
    });
  });
