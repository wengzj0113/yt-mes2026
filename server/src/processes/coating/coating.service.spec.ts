...
describe('CoatingService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CoatingService,
        { provide: getRepositoryToken(CoatingRecord), useValue: repo },
  ...
  describe('createDraft', () => {
    ...
    it('should update existing draft instead of creating new', async () => {
      ...
      (repo.save as jest.Mock).mockResolvedValue({ ...existingDraft, equipmentCode: 'CT-001' });

      const result = await service.createDraft(draftDto, 1);
      expect(result.equipmentCode).toBe('CT-001');
    });
  });
  ...
  describe('submitQuality', () => {
    ...
    it('should reject when operator fields are empty', async () => {
      ...
      (repo.findOne as jest.Mock).mockResolvedValue({
        ...
        equipmentCode: null,
        coatingSpeed: null,
        coatingThicknessPos: null,
        coatingThicknessNeg: null,
        arealDensityPos: null,
        arealDensityNeg: null,
