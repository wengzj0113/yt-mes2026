...
describe('GradingService', () => {
  ...
  let repo: jest.Mocked<Partial<Repository<GradingRecord>>>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        GradingService,
        { provide: getRepositoryToken(GradingRecord), useValue: repo },
  ...
  describe('createDraft', () => {
    ...
    it('should update existing draft instead of creating new', async () => {
      ...
      (repo.save as jest.Mock).mockResolvedValue({ ...existingDraft, equipmentCode: 'GD-001' });
      const result = await service.createDraft(draftDto, 1);
      expect(result.equipmentCode).toBe('GD-001');
    });
  });
  ...
  describe('submitQuality', () => {
    const qualityDto = {
      batchNo: 'WT26A01MA',
      chargeDischargeTemplate: '分容工步-01',
      gradingTemperature: 25.0,
      capacityGradeStandard: 'A级标准',
    };
    ...
    it('should submit draft with quality fields', async () => {
      ...
      expect(result.isDraft).toBe(false);
      expect(result.chargeDischargeTemplate).toBe('分容工步-01');
      expect(result.gradingTemperature).toBe(25.0);
      expect(result.capacityGradeStandard).toBe('A级标准');
    });
