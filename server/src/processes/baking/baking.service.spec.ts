...
describe('BakingService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BakingService,
        { provide: getRepositoryToken(BakingRecord), useValue: repo },
    ...
    service = module.get<BakingService>(BakingService);
  });
  ...
  describe('createDraft', () => {
    ...
    it('should update existing draft instead of creating new', async () => {
      const existingDraft = { ...mockRecord, id: 1, equipmentCode: 'Old-BK' };
      (batchRepo.findOne as jest.Mock).mockResolvedValue(mockBatch);
      (repo.findOne as jest.Mock).mockResolvedValue(existingDraft);
      (repo.save as jest.Mock).mockResolvedValue({ ...existingDraft, equipmentCode: 'BK-001' });

      const result = await service.createDraft(draftDto, 1);
      expect(result.equipmentCode).toBe('BK-001');
    });
  });
  ...
  describe('submitQuality', () => {
    ...
    it('should submit draft with quality fields', async () => {
      ...
      expect(result.isDraft).toBe(false);
      expect(result.vacuumLevel).toBe(-0.095);
    });
